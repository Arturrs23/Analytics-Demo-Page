          ...
          <div class="flex items-center justify-center sm:items-stretch sm:justify-start">
            ...
            <div class="hidden sm:block pl-80">
              <div class="flex space-x-4 text-button-color">
                <a v-for="item in navigation" :key="item.name" :href="item.href" :class="[item.current ? ' text-button-color' : '', item.name === 'Sign In' ? 'md:hidden' : '', item.name === 'Sign Up' ? 'md:hidden bg-green-500 text-white' : '', 'px-3 py-2 rounded-md font-medium text-base hover:bg-transparent hover:bg-sky-700 hover:text-white']" :aria-current="item.current ? 'page' : undefined">{{ item.name }}</a>
              </div>
            </div>
          </div>
